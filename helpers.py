...
def plot_bode_mimo(A_smps,B_smps,C_smps,D_smps,A_t,B_t,C_t,D_t,nu,ny,omega,no_plot=300,max_samples=1000, save=False):
    """plot bode diagram from estimated system samples and true sys"""
    ...
    for j in range(0,ny):
        for i in range(0,nu):
            ...
            for s in sel:
                ...
                C_s = C_smps[s,[j],:].reshape(-1,n_states)
                D_s = D_smps[s,[j],[i]]
                w, mag_samples[:, count], phase_samples[:, count] = signal.bode((A_s,B_s,C_s,float(D_s)), omega)
                count = count + 1

            w, mag_true, phase_true = signal.bode((A_t, B_t[:,[i]], C_t[[j],:], float(D_t[[j],[i]])), omega)
            ...
            # plt.title('Bode diagram')
            # plt.ylabel('Magnitude (dB)')
            plt.xlabel('Input '+str(i+1)+' to output '+str(j+1))
            plt.xlim((min(omega), max(omega)))
            ...
            # plt.xlim((min(omega), max(omega)))
            plt_idx = plt_idx + 1
    plt.suptitle("Mag. (dB) vs. freq. (rad/s) of linear block")
    plt.tight_layout()
    if save:
        plt.savefig('bode_plot.png',format='png')
...
def plot_phase_mimo(A_smps,B_smps,C_smps,D_smps,A_t,B_t,C_t,D_t,nu,ny,omega,no_plot=300,max_samples=1000, save=False):
    """plot bode diagram from estimated system samples and true sys"""
    # plot_bode(traces['A'],traces['B'][:,:,[0]],traces['C'][:,[0],:],traces['D'][:,[0],[0]],par.A,par.B[:,[0]],par.C[[0],:],par.D[[0],[0]],omega)
    no_samples = np.shape(A_smps)[0]
    n_states= np.shape(A_smps)[1]
    ...
    for j in range(0,ny):
        for i in range(0,nu):
            ...
            plt.xlim((min(omega), max(omega)))
            plt_idx = plt_idx + 1
    plt.suptitle("Phase (deg) vs. freq. (rad/s) of linear block")
    plt.tight_layout()
    if save:
        plt.savefig('phase_plot.png',format='png')
